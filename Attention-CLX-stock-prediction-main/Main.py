...
m = build_model(MODEL_TYPE, INPUT_DIMS=7, TIME_STEPS=TIME_STEPS)
m.summary() 
...
plt.title(MODEL_TYPE + ' Model Training and Validation Loss')
plt.legend()
plt.show()
...
name = config.dimname
# normalize = np.load("normalize.npy")
# y_hat, y_test = PredictWithData(data2, data_yuan, name, 'stock_model.weights.h5',7)
...
plt.title(MODEL_TYPE + ' model prediction')
